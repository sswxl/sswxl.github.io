...
**KB Type Representation  ** 为了获得KB类型表示，我们针对当前提及运行EL算法。 **如果EL算法返回一个实体**，我们将从KB中检索该实体的类型。 我们将Freebase用作$KB$[^5]。 由于Freebase中的类型不同于$T$（目标类型集），因此它们**使用与(Zhou et al.，2018)[^6]中相似的规则****映射到$T$中的类型**。 然后，我们对这些类型进行**独热编码以获得KB类型表示$f_e$。 如果EL算法返回NIL（即，提及内容不能链接到实体)，我们只需对空类型集进行独热编码。
